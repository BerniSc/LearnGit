...
parameterDescription thresholdMode(0, 1, 1, "Threshold Mode (Binary)");
parameterDescription blurMode(0, 1, 1, "Blur Mode");
parameterDescription blurSize(0, 7, 7, "Bluring Size (1:=3 ,2:=5 etc.)");
parameterDescription cannyLow(0, 60, 31, "Canny Threshold Low");
parameterDescription cannyHigh(0, 180, 90, "Canny Threshold High");
parameterDescription resizeSize(0, 50, 0, "Window Size (in %)");
parameterDescription displayWindowSize(0, 50, 0, "Displaywindow Size (in %)");
parameterDescription thresholdLow(0, 255, 80, "Lower Threshold Value");
parameterDescription thresholdHigh(0, 255, 255, "Higher Threshold Value");
//Changed BERNHARD SCHÖFFEL
...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        while (1)
        {
            ...
            if (thresholdModeBool)
                threshold(imageGray, imageGray, thresholdLow.getValue(), thresholdHigh.getValue(), THRESH_BINARY);
            imageBlurred = getBlurred(imageGray);
            imageCanny = getCanny(imageBlurred);
            ...
            if (warpMode)
            {
                ...
                // cout << corners.at(0) << "  " << corners.at(1) <<endl;
                // if(!outputWarped.empty()) {
                //         cout << !outputWarped.empty() << endl;
                vector<Vec3f> circles = findCircles(imageCanny, image);
                drawCircles(imageCanny, circles);
